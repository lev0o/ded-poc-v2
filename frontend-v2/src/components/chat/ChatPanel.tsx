...
function getFilteredMentionItems(catalog: any, query: string): ContextItem[] {
  ...
  catalog.workspaces.forEach((ws: any) => {
    items.push({ label: "Workspace", id: ws.id });
  });
  ...
  return (
    <div className="flex h-full flex-col bg-[var(--color-bg-secondary)]">
      ...
      <div className="flex-shrink-0 flex items-center justify-between px-3 py-2 bg-[var(--color-bg-secondary)] h-10">
        ...
        <button
          onClick={onNewChat}
          className="text-xs rounded px-2 py-1 transition-colors font-medium bg-[var(--color-bg-tertiary)] text-[var(--color-text-primary)] hover:bg-[var(--color-bg-elevated)] whitespace-nowrap"
        >
          New chat
        </button>
      </div>
      ...
      {/* Fixed Info Banner */}
      <div className="flex-shrink-0 bg-[var(--color-bg-tertiary)] py-2 flex items-center justify-center">
        <button
      ...
      <div className="p-3 flex-shrink-0">
        <div className="relative">
          <textarea
            ...
            className={`w-full h-20 resize-none rounded border border-[var(--color-border-primary)] bg-[var(--color-bg-tertiary)] text-[var(--color-text-primary)] p-2 focus:border-[var(--color-info)] focus:ring-2 focus:ring-[var(--color-info)] transition-colors placeholder:text-xs placeholder:italic placeholder:text-[var(--color-text-tertiary)] ${isCatalogRefreshing ? 'opacity-60 cursor-not-allowed' : ''}`}
          />
        ...
        <div className="mt-2 flex justify-between items-center">
          ...
          <div className="ml-3">
            <button
              ...
              onClick={handleSend}
              className="w-8 h-8 rounded-full bg-[var(--color-info)] text-white disabled:opacity-60 hover:bg-[var(--color-info)]/80 transition-colors shadow-sm flex items-center justify-center"
            >
            {isPending ? (
          ...
          <div className="bg-[var(--color-bg-primary)] border border-[var(--color-border-primary)] rounded-lg p-6 max-w-md mx-4">
            ...
            <div className="space-y-3 text-sm text-[var(--color-text-primary)]">
              ...
              <div className="mt-4 p-3 bg-[var(--color-bg-tertiary)] rounded border border-[var(--color-border-primary)]">
                ...
                <p className="text-xs mt-1 text-green-500">Try: "Forecast sales for the next 6 months" or "Predict customer growth trends"</p>
              </div>
            </div>
          </div>
        </div>
